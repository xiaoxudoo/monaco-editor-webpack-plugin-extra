...
function getCompilerHook(compiler, {id, entry, filename, publicPath, chunkFilename, plugins}) {
  return function(compilation, callback) {
    const outputOptions = {
      filename,
      chunkFilename,
      publicPath: publicPath || compilation.outputOptions.publicPath,
...
class AddWorkerEntryPointPlugin {
  constructor({
    ...
    this.options = { id, entry, filename, publicPath, chunkFilename, plugins };
  }
